...
public class UserRepository implements IUserRepository {
    ...
    public Optional<UserDetail> findByUsername(String username) {
        ...
        try (PreparedStatement ps = db.getConnection().prepareStatement(sql)) {
            ...
            System.err.println("[UserRepository] findByUsername: " + e.getMessage());
        }
        return Optional.empty();
    }

    public java.util.List<UserDetail> getAllUsers() {
        String sql = "SELECT * FROM users ORDER BY username";
        java.util.List<UserDetail> users = new java.util.ArrayList<>();
        try (PreparedStatement ps = db.getConnection().prepareStatement(sql)) {
            ResultSet rs = ps.executeQuery();
            while (rs.next()) users.add(map(rs));
        } catch (SQLException e) {
            System.err.println("[UserRepository] getAllUsers: " + e.getMessage());
        }
        return users;
    }
